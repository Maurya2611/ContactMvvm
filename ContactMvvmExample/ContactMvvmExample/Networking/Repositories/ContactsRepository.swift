...
class ContactsRepository: Repository<Contact> {
    ...
    override func getList(params: Contact?, completion: @escaping ArrayC) {
        super.getList(params: params, completion: completion)
        let path = Paths.contacts
        let request = Request(path: path, method: .get)
        createSuccessAndFail(request, completion: completion) { (contacts, group) in
    ...
    override func get<U: LosslessStringConvertible>(params: U?,
        ...
        let path = Paths.contact.replacingOccurrences(of: URLParameters.contactId,
                                                      with: String(param))
        let request = Request(path: path, method: .get)
        createSuccessAndFail(request, completion: completion)
        requests.append(request)
    }
    override func edit(params: Contact?,
                       completion: @escaping ((Contact?, Error?) -> Void)) {
        super.edit(params: params, completion: completion)
        guard let contact = params else { return }
        let path = Paths.contact.replacingOccurrences(of: URLParameters.contactId,
                                                      with: String(contact.id ?? 0))
        let request = Request(path: path, method: .put)
    ...
    override func delete<U: LosslessStringConvertible>(params: U?,
        ...
        let path = Paths.contact.replacingOccurrences(of: URLParameters.contactId,
                                                      with: String(params))
        let request = Request(path: path, method: .delete)
        createSuccessAndFail(request, completion: completion)
        requests.append(request)
    }
}
